...
export default function DiscountSort({ setTextSearch, discounts, isSearching, SORTS_DISCOUNT, selectedFilters, handleChangeSelectedFilters, setIsSearching, setDiscounts }: DiscountSortProps) {
  const getFieldType = (field: string) => {
    if (["Titulo"].includes(field)) {
      return "string";
    } if (["Creado", "Fecha de inicio", "Fecha de finalización"].includes(field)) {
      return "date";
    } else {
      return "number";
    }
  ...
  return (
    <div className="flex items-center space-x-2">
      <Button
        ...
        {!isSearching && (
          <>
            <SearchIcon className="text-[#4a4a4a] md:size-4 size-5 stroke-2" />
            <SortIcon className="md:size-4 size-5 fill-[#4a4a4a] stroke-none" />
          </>
        )}
      </Button>
