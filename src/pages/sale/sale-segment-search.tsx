...
interface SaleSegmentSearchProps {
...
export default function SaleSegmentSearch({ setTextSearch, textSearch, inputRef, isSearching, sales, selectedFilters, setSales, setIsLoading, handleChangeSelectedFilters }: SaleSegmentSearchProps) {
  const SEGMENTS = ["Todos", "Contados", "Créditos", "Anulados"];
  ...
  const handleSegmentSales = async (segment: number) => {
    ...
    if (segment === 0) {
      ...
      setSales(sales.filter(sale => sale.typeSale === 1));
    } else if (segment === 2) {
      setSales(sales.filter(sale => sale.typeSale === 0));
    } else if (segment === 3) {
      setSales(sales.filter(sale => sale.state === 0));
    }
        ...
        <div className="border border-transparent rounded-lg flex items-center px-3 h-full py-1 w-full group focus-within:border-blue-500">
          <div className='flex items-center space-x-2 w-full'>
            ...
            <input
              ...
              placeholder="Buscando en todos las ventas"
