...
interface CashBoxSortProps {
  cashBoxes: CashBoxProps[];
  inputRef: React.RefObject<HTMLInputElement | null>;
  isSearching: boolean;
  SORTS_CASH_BOXES: string[];
...
export default function CashBoxSort({ SORTS_CASH_BOXES, cashBoxes, setCashBoxes, isSearching, selectedFilters, handleChangeSelectedFilters, setIsSearching, inputRef }: CashBoxSortProps) {
  const getFieldType = (field: string) => {
    if (["Titulo de la caja", "Estado"].includes(field)) {
      return "string";
    ...
    <div className="flex items-center space-x-2">
      ...
      <Dropdown closeToClickOption={false}>
        ...
        <DropdownContent align="end">
          {SORTS_CASH_BOXES.map((sort, index) => (
