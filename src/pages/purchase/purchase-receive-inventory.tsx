...
export default function PurchaseReceiveInventory() {
  ...
  const [formData, setFormData] =
    useState<PurchaseOrderReceiptProps>(INITIAL_PRODUCTS);
  const [productStatus, setProductStatus] = useState<
    { productId: string; accepted: number; rejected: number }[]
  >([]);
  const { userPermissions } = useRolePermission();
  const permissions = userPermissions?.permissions || null;
  const canReceiveInventory = getPermissions(
    permissions,
    'Ordenes de compras',
    'Recibir inventario',
  )?.canAccess;
    ...
    const handleLoadPurchaseById = async () => {
      ...
      setProductStatus(
        purchaseDate.products.map((product) => ({
          ...
          rejected: Math.max(
            0,
            product.orderedQuantity - product.receivedQuantity,
          ),
        })),
      );
    };
  ...
  const handleSavePurchaseReceiptDetail = async (purchaseReceiptId: string) => {
    ...
    const { error } = await supabase
      .from('purchaseReceiptDetail')
      .insert(productsReceipt);
  ...
  const handleChangeInventory = async () => {
    for (const product of productStatus) {
      // 1. LÓGICA DE INVENTARIO (La que ya tenías)
      ...
      if (product.accepted > 0) {
        ...
        if (currentCost > 0) {
          ...
          if (updateCostError) {
            showToast(
              `Error al actualizar costo del producto: ${updateCostError.message}`,
              false,
            );
          }
        }
    ...
    >
      {canReceiveInventory ? (
        <section className="max-w-[950px] flex flex-1 flex-col mt-2 w-full mx-auto">
          <SubHeader
            backUrl={`/purchases/receive/${purchaseId}`}
            ...
            <FormSection className="px-0">
              <>
                <header className="flex px-4 md:flex-row flex-col md:items-center md:justify-between">
                  <h2 className="text-secondary font-semibold md:text-sm text-base">
                    Productos
                  </h2>
                  {formData.state < 2 && (
                    <div className="flex items-center md:justify-start justify-between space-x-2 md:mt-0 mt-4">
                      <Button
                        onClick={() => handleChangeOption('accept')}
                        type="button"
                        name="Aceptar todo"
                        styleButton="none"
                        className="md:text-2xs text-base font-medium text-blueprimary hover:text-bluesecondary hover:underline"
                      />
                      <Button
                        onClick={() => handleChangeOption('reject')}
                        type="button"
                        name="Rechazar todo"
                        styleButton="none"
                        className="md:text-2xs text-base font-medium text-blueprimary hover:text-bluesecondary hover:underline"
                  ...
                  {isShowModal && (
                    ...
                    >
                      <p className="font-medium text-2xs text-secondary/90 p-4">
                        Esta acción marcará todas las unidades como{' '}
                        {selectedOption === 'accept'
                          ? 'aceptadas'
                          : 'rechazadas'}
                        , guardará la orden de compra y actualizará el
                        inventario.
                      </p>
                    </Modal>
                  )}
                ...
                <main className="mt-6">
                  <ProgressBar
                    ...
                    total={
                      formData?.products.reduce(
                        (acc, product) => acc + product.orderedQuantity,
                        0,
                      ) ?? 0
                    }
                    value1={productStatus.reduce(
                      (acc, status) => acc + status.accepted,
                      0,
                    )}
                    value2={productStatus.reduce(
                      (acc, status) => acc + status.rejected,
                      0,
                    )}
                  />
                    ...
                    <table className="w-full text-left">
                      ...
                      <tbody>
                        {formData?.products.map(
                          ({ productId, productName, orderedQuantity }) => (
                            <tr
                              key={productId}
                              className="bg-white border-b font-medium text-sm border-gray-200"
                            >
                              <th className="px-4 py-4 w-[30%] text-blueprimary hover:underline font-medium cursor-pointer">
                                <Link to={`/products/add/${productId}`}>
                                  {productName}
                                </Link>
                              </th>
                              <td className="px-2 py-4">
                                <div className="flex items-center space-x-2">
                                  ...
                                  <Button
                                    ...
                                    styleButton="primary"
                                    type="button"
                                    className="px-3 py-1 text-2xs disabled:shadow-none disabled:cursor-default disabled:bg-[#f2f2f2] disabled:text-[#beb8b5] font-semibold text-secondary/80"
                                  />
                                </div>
                              </td>
                              <td className="px-2 py-4">
                  ...
                  <div className="px-4 mt-6 border-y border-gray-300 py-5 md:hidden block">
                    {formData?.products.map(
                      ({ productId, productName, orderedQuantity }) => (
                        <div key={productId}>
                          <Link
                            to={`/products/add/${productId}`}
                            className="font-medium text-blueprimary hover:underline hover:text-bluesecondary"
                          >
                            {productName}
                          </Link>
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  handleInputChange(productId, 'rejected', 0);
                                }}
                                name="Todo"
                                styleButton="primary"
                                type="button"
                                className="px-4 py-1 text-2xs h-11 shadow-none active:shadow-none border disabled:shadow-none disabled:cursor-default disabled:bg-[#f2f2f2] disabled:text-[#beb8b5] font-semibold text-secondary/80"
                              />
                            </div>
                            <div className="flex items-center space-x-2">
                              <FieldInput
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  handleInputChange(productId, 'accepted', 0);
                                }}
                                name="Todo"
                                styleButton="primary"
                                type="button"
                  ...
                  <span className="font-medium md:text-2xs text-base text-secondary/90 inline-block mx-4 mt-4">
                    {formData?.products.length} productos en la orden de compra
                  </span>
                </main>
              </>
            </FormSection>
