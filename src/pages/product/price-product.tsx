...
interface PriceProductProps {
  handleChangeFormData: (
    name: keyof Omit<
      ...
      | 'nameSupplier'
    >,
    value: string | number | string[] | boolean | ProductSupplierProps[],
  ) => void;
  formData: Omit<
    ProductProps,
    | 'productId'
...
export default function PriceProduct({
  ...
  const canViewCost = getPermissions(
    permissions,
    'Productos',
    'Ver',
    'Ver costo',
  )?.canAccess;
  const canViewAdjustmentHistory = getPermissions(
    permissions,
    'Inventario',
    'Ver movimientos',
  )?.canAccess;
  ...
  const handleInputChange = (
    field: keyof ProductSupplierProps,
    value: string,
    supplier: ProductSupplierProps,
  ) => {
    const numericValue = value.trim() === '' ? 0 : parseFloat(value);

    const updatedSupplier = { ...supplier, [field]: numericValue };

    if (field === 'cost') {
      // Precio mínimo: Costo + 15%
      updatedSupplier.minPrice = Number((numericValue * 1.15).toFixed(2));

      // Precio base automático: Costo + 20% (Nuevo cálculo)
      updatedSupplier.price = Number((numericValue * 1.2).toFixed(2));

      // Precio sugerido: Costo + 40%
      updatedSupplier.suggestedPrice = Number((numericValue * 1.4).toFixed(2));
    }

    handleChangeFormData(
      'suppliers',
      formData.suppliers.map((sup) =>
        sup.supplierId === supplier.supplierId ? updatedSupplier : sup,
      ),
    );
  };
  ...
  const handleRemoveSupplier = (supplier: ProductSupplierProps) => {
    handleChangeFormData(
      'suppliers',
      formData.suppliers.filter(
        (sup) => sup.supplierId !== supplier.supplierId,
      ),
    );
  };
        ...
        <FormSection className="px-0 pb-2">
          <>
            <div className="flex items-center justify-between px-4">
              <h2 className="font-[550] md:text-sm text-base text-secondary/90 mb-3">
                Fijar precio
              </h2>
              {canViewAdjustmentHistory && isEditing && (
                <Link
                  to={`/products/add/${product_id}/adjustment_history?productName=${formData.name}`}
                  className="text-blueprimary font-medium md:text-2xs text-base hover:underline hover:text-bluesecondary"
                >
                ...
                <div className="flex border-y pt-3 border-gray-200">
                  {tabs.map((tab) => (
                    ...
                    >
                      {tab}
                      <div
                        className={`absolute bottom-0 left-0 w-full h-0.5 rounded-t-lg transition-all ${activeTab === tab ? 'bg-black' : 'bg-transparent group-hover:bg-[#bababa]'}`}
                      />
                    </button>
                  ))}
                </div>
            ...
            <div
              className={`relative md:block hidden overflow-x-auto ${canViewCost ? 'mt-2' : ''}  px-px`}
            >
                ...
                <div key={index}>
                  <div className="flex items-center justify-between">
                    <Link
                      ...
                      className="text-blueprimary hover:underline text-base font-medium cursor-pointer"
                    >
                      {suppliers.find(
                        (sup) => sup.supplierId === supplier.supplierId,
                      )?.nameSupplier ?? ''}
                  ...
                  <div className="flex items-center space-x-2 mt-4 [&>div]:w-full">
                    <FieldInput
                      isNumber
                      name="Precio (Automático)"
                      className="mb-0 bg-gray-100 cursor-not-allowed" // Fondo gris
                      value={supplier.price || ''}
                      disabled={true} // Bloqueamos la entrada manual
                    />
                    <FieldInput
                      isNumber
                      className="mb-0 bg-gray-100 cursor-not-allowed"
                      name="Precio sugerido (Automático)"
                  ...
                  <div className="flex items-center space-x-2 mt-4 [&>div]:w-full">
                    ...
                    <FieldInput
                      isNumber
                      name="Costo" 
                      className="mb-0 border-blueprimary" 
                      value={supplier.cost || ''}
                      onChange={(e) =>
