...
export default function AddSupplier() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const validations = [
      ...
      },
      {
        field: 'socialReason',
        label: 'razón social',
        message: 'Ya existe un proveedor con esta razón social.',
      },
      {
        field: 'email',
        label: 'correo',
  ...
  return (
    <Container
      ...
      isLoading={isLoading}
      onClickSecondary={() => navigate('/suppliers/')}
    >
      <>
        <section className="flex flex-col items-center h-full flex-1">
          <div className="max-w-screen-lg mt-2 w-full mx-auto flex-1 flex flex-col">
            <SubHeader
              title={!supplierId ? 'Añadir proveedor' : supplierName}
              backUrl="/suppliers"
              isShowButtons={supplierId ? true : false}
              status={formData.state ? true : false}
            />
            <form
                ...
                <FormSection name="Información del proveedor">
                  <>
                    <FieldInput
                      ...
                      onChange={(e) =>
                        handleChangeFormState('nameSupplier', e.target.value)
                      }
                      name="Nombre"
                      id="nameSupplier"
                      className="mb-5"
                    ...
                    <div className="flex items-center w-full mt-5">
                      <FieldInput
                        ...
                        value={formData.ruc}
                        onChange={(e) =>
                          handleChangeFormState('ruc', e.target.value)
                        }
                        name="RUC"
                        id="ruc"
                      ...
                      <FieldInput
                        ...
                        onChange={(e) =>
                          handleChangeFormState('socialReason', e.target.value)
                        }
                        name="Razón Social"
                        id="SocialReason"
                ...
                <FormSection name="Metodos de contacto">
                  <>
                    <FieldInput
                      ...
                      onChange={(e) =>
                        handleChangeFormState('email', e.target.value)
                      }
                      name="Correo"
                      id="email"
                      className="mb-5"
