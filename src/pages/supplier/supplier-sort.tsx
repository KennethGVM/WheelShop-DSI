...
interface SupplierSortProps {
  suppliers: SupplierProps[];
  isSearching: boolean;
  SORTS_SUPPLIERS: string[];
...
export default function SupplierSort({ setTextSearch, suppliers, isSearching, SORTS_SUPPLIERS, selectedFilters, handleChangeSelectedFilters, setIsSearching, setSuppliers }: SupplierSortProps) {
  const getFieldType = (field: string) => {
    if (["Nombre del proveedor", "Razon social", "RUC"].includes(field)) {
      return "string";
    } if (["Creado", "Estado"].includes(field)) {
      return "date";
    } else {
      return "number";
    }
    ...
    <div className="flex items-center space-x-2">
      ...
      <Dropdown closeToClickOption={false}>
        ...
        <DropdownContent align="end">
          {SORTS_SUPPLIERS.map((sort, index) => (
