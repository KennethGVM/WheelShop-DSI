...
export default function Home() {
  ...
  const [selectedStoreHouse, setSelectedStoreHouse] =
    useState<StoreHouseProps | null>(null);
  ...
  useEffect(() => {
    const handleLoadTotalInventory = async () => {
      const { data } = await supabase
        .from('get_total_inventory')
        .select('*')
        .eq('storeHouseId', selectedStoreHouse?.storeHouseId)
        .single();
      if (!data || data.length === 0) return;
      ...
      setTypeReportTotals([
        {
          name: 'Ventas totales',
          value:
            (data.sale?.reduce((a, b) => a + Number(b.total), 0) ?? 0) -
            (data.comissions?.reduce((a, b) => a + Number(b.total), 0) ?? 0),
        },
        {
          name: 'Pedidos',
          value: data.purchase?.reduce((a, b) => a + Number(b.total), 0) ?? 0,
        },
        {
          name: 'Gastos',
          value: data.expense?.reduce((a, b) => a + Number(b.total), 0) ?? 0,
        },
        {
          name: 'Devoluciones',
          value: data.return?.reduce((a, b) => a + Number(b.total), 0) ?? 0,
        },
        {
          name: 'Comisiones',
          value: data.comissions?.reduce((a, b) => a + Number(b.total), 0) ?? 0,
        },
      ]);
    };

    const handleLoadMostProduct = async () => {
      const { data, error } = await supabase.rpc('getproductmostsale', {
  ...
  const selectedDataKey = [
    ...
    'return',
    'comissions',
  ][selectedType] as keyof TotalsGroupedByType;

  const lineChartData =
    report && report[selectedDataKey]
      ? [
          {
            name: typeReportTotals[selectedType].name,
            data:
              report[selectedDataKey]?.map(({ date, total }) => ({
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <FormSection className="p-0">
            <div className="flex items-center">
                  ...
                  <DropdownTrigger>
                    <Button className="flex items-center space-x-1.5 px-4 py-3 text-secondary/80 md:text-2xs text-base font-[550] bg-white hover:bg-[#f3f3f3]">
                      ...
                      <span className="font-medium">
                        {selectedStoreHouse?.name}
                      </span>
                    </Button>
                  </DropdownTrigger>
                  <DropdownContent
                    className="w-60 rounded-xl mt-1 space-y-1"
                    align="start"
                  >
                    {storeHouse.map((storeHouse) => (
                      <DropdownItem
                        ...
                        className={`flex items-center justify-between ${
                          selectedStoreHouse?.storeHouseId ===
              ...
              <div
                className="flex w-full overflow-x-auto [&>div>span]:text-nowrap px-2 gap-2 flex-grow"
                ...
                <div className="min-w-[600px] flex items-center gap-4 md:mb-5 mb-10">
                  {typeReportTotals.map((typeReport, index) => (
                    <div
                      ...
                      key={index}
                      className={`px-4 w-56 py-2 cursor-pointer rounded-lg ${
                        selectedType === index ? 'bg-[#f1f1f1]' : ''
                      }`}
                    >
          ...
          <div className="flex md:flex-row flex-col items-center md:space-x-4 md:space-y-0 space-y-4 mt-4">
            ...
            <FormSection
              className="md:w-1/2 w-full"
              name="Acumulado de productos (pareto)"
              classNameLabel="mb-10"
            >
              <BarChart
                theme="light"
              ...
              <table className="w-full text-left">
                ...
                <tbody>
                  {topClients &&
                    topClients.map(
                      (
                        { cliente, total, producto_favorito, monto_producto },
                        key,
                      ) => (
                        <tr
                          key={key}
                        ...
                        >
                          ...
                          <td className="px-2 py-4 text-right">
                            {currencyFormatter(monto_producto)}
                          </td>
                          <td className="px-2 py-4 text-right">
