...
interface PendingAccountFiltersProps {
  isSearching: boolean;
...
export default function PendingAccountFilters({ isSearching, pendingAccounts, filters, setFilters }: PendingAccountFiltersProps) {
  ...
  return (
    <div className={`overflow-hidden ${pendingAccounts && pendingAccounts.length > 0 ? 'border-0' : 'border-b border-gray-300'} transition-all duration-200 ease-out ${isSearching ? "max-h-20 opacity-100" : "max-h-0 opacity-0"}`}>
