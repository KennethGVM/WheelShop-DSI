...
interface PenndingAccountSortProps {
  pendingAccounts: PendingAccountProps[];
  isSearching: boolean;
  SORTS_PENDING_ACCOUNT: string[];
...
export default function PenndingAccountSort({ setTextSearch, pendingAccounts, isSearching, SORTS_PENDING_ACCOUNT, selectedFilters, handleChangeSelectedFilters, setIsSearching, setPendingAccounts }: PenndingAccountSortProps) {
  const getFieldType = (field: string) => {
    if (["Cliente", "Codigo de la venta"].includes(field)) {
      return "string";
    } if (["Fecha", "Vencimiento"].includes(field)) {
      return "date";
    } else {
      return "number";
    }
    ...
    <div className="flex items-center space-x-2">
      ...
      <Dropdown closeToClickOption={false}>
        ...
        <DropdownContent align="end">
          {SORTS_PENDING_ACCOUNT.map((sort, index) => (
