...
interface UserPermissionSortProps {
  users: UserPermissionProps[];
  inputRef: React.RefObject<HTMLInputElement | null>;
  isSearching: boolean;
  SORTS_USER_PERMISSION: string[];
...
export default function UserPermissionSort({ users, isSearching, SORTS_USER_PERMISSION, selectedFilters, handleChangeSelectedFilters, setIsSearching, inputRef, setUsers }: UserPermissionSortProps) {
  const getFieldType = (field: string) => {
    if (["Nombre de usuario", "Estado"].includes(field)) {
      return "string";
    } if (["Ultimo inicio de sesión", "Fecha de creación", "Ultima actualización"].includes(field)) {
      return "date";
    } else {
      return "number";
    }
    ...
    <div className="flex items-center space-x-2">
      ...
      <Dropdown closeToClickOption={false}>
        ...
        <DropdownContent align="end">
          {SORTS_USER_PERMISSION.map((sort, index) => (
