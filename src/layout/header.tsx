...
export default function Header({
  ...
  return (
    <header className="[grid-area:header] flex justify-between items-center bg-primary py-2.5 md:px-6 px-4">
      <img
        src="/images/hmc-logo.jpg"
        className="w-32 lg:block hidden h-auto rounded-md"
        ...
        {!save ? (
          <>
            <div className="flex items-center space-x-1 px-2">
              ...
              <span className="md:text-2xs text-base font-medium text-[#b5b5b5]">
                Buscar
              </span>
            </div>

            <div className="px-2 md:flex hidden items-center space-x-1">
              <span className="font-medium bg-[#373737] p-1 rounded-md text-whiting2/80 text-[10px]">
                CTRL
              </span>
              <span className="font-medium bg-[#373737] px-2 py-1 rounded-md text-whiting2/80 text-[10px]">
                K
              </span>
            </div>
          </>
                ...
                {isLoading && (
                  <div role="status">
                    <svg
                      ...
                      aria-hidden="true"
                      className="size-5 text-transparent animate-spin fill-secondary/50"
                      viewBox="0 0 20 20"
                    >
        ...
        <Dropdown>
          <DropdownTrigger>
            <button className="flex rounded-full space-x-4 justify-between items-center lg:px-2 lg:py-1 bg-secondary">
              ...
              />
              <h3 className="font-bold lg:block hidden text-sm text-whiting2 ml-3">
                {name}
              </h3>
            </button>
          </DropdownTrigger>
          <DropdownContent
            align="end"
            className="w-[350px] space-y-1 rounded-2xl"
          >
            ...
            >
              <p className="text-sm">Gestionar cuenta</p>
            </DropdownItem>
